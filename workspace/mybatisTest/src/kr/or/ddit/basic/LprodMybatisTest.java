...
public class LprodMybatisTest {
	...
	public static void main(String[] args) {
		...
		// 1. myBatis의 환경설정파일(mybatis-config.xml)
		InputStream in = null;
		SqlSessionFactory sqlSessionFactory = null;
		try {
			// 1-1. 환경설정파일을 읽어올 스트림 객체 생성
			in = Resources.getResourceAsStream("kr/or/ddit/mybatis/config/mybatis-config.xml");
			
			// 1-2 확경 설정 파일을 읽어와 환경 설정을 완성한 후 SQL문을 호출해서 실행할 수 있는
			// 객체를 생성할 수 SQLSessionFacotry 객체를 생성한다.
...
//			// 형식) SqlSessionFacotry객체.openSession(논리값)
//    		//		 '논리값'이 true이면 AutoCommit이 활성된 상태이고
//			//		 '논리값'이 생략되거나 false이면 AutoCommit이 바활성화된 상태이다
//			session = sqlSessionFactory.openSession();
//			
//			// SqlSession객체를 이용하여 처리한 SQL문을 호출하여 실행한다.
...
//			// 반환값: 작업에 성공한 레코드 수
//			int deleteCnt = session.delete("lprod.deleteLprod", lproodGu3);
//			if(deleteCnt > 0) {
...
//	         session = sqlSessionFactory.openSession();
//	         
//	         // 응답 결과가 여러 개일 경우 selectList()메서드를 사용하는데 이 메서드는
//	         // 여러 개의 레코드 각각을 VO객체에 담은 후 VO데이터를 List에 추가해 주는
//	         // 작업을 자동으로 수행한다.
