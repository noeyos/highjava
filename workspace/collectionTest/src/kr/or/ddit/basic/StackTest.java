...
class Browser {
	...
	private String currentURL;		// 현재 페이지의 주소가 저장될 변수
	...
	// 뒤로가기
	public void goBack() {
		// isEmpty()메서드 ==> 컬렉션이 비어있으면 true. list 등에서도 사용 가능한 명령
		if(!back.isEmpty()) {	// 비어있는지 안 비어있는지 검사
			forward.push(currentURL);	// 현재 페이지를 forward 스택에 추가한다.
			currentURL = back.pop();	// back 스택에서 1개의 요소를 꺼내와 현재 페이지로 설정.
		}
	}
