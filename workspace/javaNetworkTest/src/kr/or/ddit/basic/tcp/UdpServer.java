...
/*
	UDP방식 : 비연결 지향, 신뢰성 없다, 데이터가 순서대로 도착한다는 보장이 없다.
			  그렇지만 TCP방식보다 속도가 빠르다.
			  
	DatagramSocekt객체와 DatagramPacket객체를 이용해서 통신한다.
	- DatagramSocket : 데이터의 송수신과 관련된 작업을 수행한다. (우체부)
	- DatagramPacket : 주고 받는 데이터와 관련된 작업을 수행한다. (소포, 편지)
	  ==> 수신용 생정자와 송신용 생성자를 따로 제공한다.
	...
	public static void main(String[] args) {
		try {
			...
			while(true) {
				...
				// '수신용패킷객체'를 생성할 때 지정한 byte형 배열에도 저장된다.
...
//				String msg = new String(bMsg, 0, inpacket.getLength(), "utf-8");
				...
				// ==> 전송할 데이터의 길이, 상대방 주소 정보, 포트번호 이렇게 4가지를 지정하여 생성한다.
