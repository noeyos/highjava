...
public class CookieAdd extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 형식) Cookie cookie변수는 = new Cookie("쿠키이름", "쿠키값");
		//    ==> '쿠키값'으로 한글을 사용할 경우에는 URLEncoder.encode()메서드로 인코딩한 후 저장한다.
		//	  ==> 하나의 쿠키는 최대 4KB(4096bytes)까지 저장 가능
		//	  ==> 하나의 도메인 당 20개 (총 300개)까지 저장 가능
		
		Cookie namCookie = new Cookie("name", URLEncoder.encode("홍길동", "utf-8"));
...
/*		Cookie ageCookie = new Cookie("age", 26+"");
		...
		쿠키변수.setMaxAge(유지시간);		==> 단위(초)
											 * -1(기본값) : 웹브라우저가 종료되면 삭제(종료될 때까지 유지)
											 * 0 : 즉시 삭제
