...
/*
  ...
  - 만드는 방법  ==>  열거형은 클래스처럼 독립된 java파일에 만들 수 있고, (1)
					  하나의 java파일에 클래스와 같이 만들 수 있고, (2)
					  클래스 안의 내부 클래스처럼 만들 수 있다. (3)
   ...
   - 열거형 선언하기
   	 ...
   	 방법2)  ==>  열거형 상수에 특정한 값을 지정해서 만들고 싶을 때...
   	 	 enum 열거형이름 {
   	 	 	  ...
   	 	 	  // 열거형의 생성자의 매개변수는 '값들'과 개수가 같고 자료형이 맞아야 한다.
...
public class EnumTest {
	...
	public static void main(String[] args) {
		...
		*/
		...
		System.out.println("mycnt ordianl ==> " + mycnt.ordinal());
		...
		*/
		...
		// switch문의 case에 지정하는 상수값은 '열거형이름'을 생략한 '상수 이름'만 지정해야 한다.
		switch(mycnt) {
		case ONE : // [정답] 열거형 이름 생략해야 됨.
//		case Count.ONE : [오류] 열거형이름을 생략하지 않음.
		...
		for(Season time : Season.values()) {
			System.out.println(time + " == " + time.name() + " --> " + time.getSpan() + ", 평균기온: " + time.getTemp());
		}
